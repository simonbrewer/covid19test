...
## -------------------------------------------------------------------------------------------
parGrid = expand.grid(nprune = 16, degree = 3)
...
for (i in 1:nstates) {
  ...
  p1 = ggscatter(mydf, x = "obs", y = "pred", col = "type", 
    ...
    geom_abline()
  ggsave(file = paste0("./statecv/mars/covid_",states[i],"_mars.png"), 
         p1, device = "png")
}
...
save(mod_results, base_results, file = "./statecv/mars/results.RData")
