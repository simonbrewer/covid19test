...
for (i in 1:nstates) {
  ...
  ## Plot tuning results
  # ggplot(modFit)
  pred_test <- predict(modFit, newdata = testing, allow.new.levels = TRUE)
  ...
  p1 = ggscatter(mydf, x = "obs", y = "pred", col = "type", 
    ...
    geom_abline()
  ggsave(file = paste0("./statecv/lme/covid_",states[i],"_lme.png"), 
         p1, device = "png")
}
...
save(mod_results, base_results, file = "./statecv/lme/results.RData")
