...
f1 <- test_rate ~ lpState_popn + 
  pcaseNew + daysSinceC + pdeathNew + daysSinceD
...
## -------------------------------------------------------------------------------------------
parGrid = expand.grid(mtry = 2, splitrule = "variance", min.node.size = 4)
...
for (i in 1:nstates) {
  ...
  p1 = ggscatter(mydf, x = "obs", y = "pred", col = "type", 
    ...
    geom_abline()
  ggsave(file = paste0("./plots/covid_",states[i],"_ranger.png"), 
         p1, device = "png")
}
